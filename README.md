# Functional UI Automation Framework - Parent Portal Website

This UI Automation framework repository has some basic functional tests for [Parent Portal](https://sisdev3.macnet.ca/home). It covers the login, add children and school program registrations. The reports of test execution published in [Github Page]() when the tests are executed from Github Actions.
...
- [The pages package](./src/main/java/pages/) contains page objects and functional methods of each page 
  ...
  - Home page - home page objects and naviagtion functional method
  - Full Time School pages - Parent information page objects, Student information page objects and  finance&documents upload page objetcs with associated functionality method
...
- The test [fulltimeSchoolRegistrationTest](./src/test/java/testCases/fullTimeSchool/fulltimeSchoolRegistrationTest.java) are designed to use the playwright feature **Storage State**. The previous login state is used in this test
...
- Logger is designed to support parallel execution and the logs will be printed with the **Thread Id**. 
  ![Sample-Log4j-logs](./readme-screenshots/samplelogs.PNG)
...
- For each test class (considered as scenario) one ExtentTest is created and for each test under the scenario class, the Extent testNode is created. As a result the report looks like below. 
  ![Extent Report](./readme-screenshots/ExtentReportSample.png)
